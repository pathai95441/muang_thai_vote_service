...
func testCandidateToManyVoteCandidateUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.VoteCandidateID, a.ID)
	queries.Assign(&c.VoteCandidateID, a.ID)
	...
	check, err := a.VoteCandidateUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if queries.Equal(v.VoteCandidateID, b.VoteCandidateID) {
			bFound = true
		}
		if queries.Equal(v.VoteCandidateID, c.VoteCandidateID) {
			cFound = true
		}
	}
	...
	if err = a.L.LoadVoteCandidateUsers(ctx, tx, false, (*[]*Candidate)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.VoteCandidateUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.VoteCandidateUsers = nil
	if err = a.L.LoadVoteCandidateUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.VoteCandidateUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCandidateToManyAddOpVoteCandidateUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddVoteCandidateUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.ID, first.VoteCandidateID) {
			t.Error("foreign key was wrong value", a.ID, first.VoteCandidateID)
		}
		if !queries.Equal(a.ID, second.VoteCandidateID) {
			t.Error("foreign key was wrong value", a.ID, second.VoteCandidateID)
		}

		if first.R.VoteCandidate != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.VoteCandidate != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.VoteCandidateUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.VoteCandidateUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.VoteCandidateUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testCandidateToManySetOpVoteCandidateUsers(t *testing.T) {
	var err error
	...
	err = a.SetVoteCandidateUsers(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetVoteCandidateUsers(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.VoteCandidateID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.VoteCandidateID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.VoteCandidateID) {
		t.Error("foreign key was wrong value", a.ID, d.VoteCandidateID)
	}
	if !queries.Equal(a.ID, e.VoteCandidateID) {
		t.Error("foreign key was wrong value", a.ID, e.VoteCandidateID)
	}

	if b.R.VoteCandidate != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.VoteCandidate != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.VoteCandidate != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
	if e.R.VoteCandidate != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}

	if a.R.VoteCandidateUsers[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.VoteCandidateUsers[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testCandidateToManyRemoveOpVoteCandidateUsers(t *testing.T) {
	var err error
	...
	err = a.AddVoteCandidateUsers(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveVoteCandidateUsers(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.VoteCandidateUsers) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.VoteCandidateUsers[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.VoteCandidateUsers[0] != &e {
