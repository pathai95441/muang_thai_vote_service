...
var CandidateRels = struct {
	VoteCandidateUsers string
	VoteHistories      string
}{
	VoteCandidateUsers: "VoteCandidateUsers",
	VoteHistories:      "VoteHistories",
}

// candidateR is where relationships are stored.
type candidateR struct {
	VoteCandidateUsers UserSlice        `boil:"VoteCandidateUsers" json:"VoteCandidateUsers" toml:"VoteCandidateUsers" yaml:"VoteCandidateUsers"`
...
func (r *candidateR) GetVoteCandidateUsers() UserSlice {
	if r == nil {
		return nil
	}
	return r.VoteCandidateUsers
}
...
// VoteCandidateUsers retrieves all the user's Users with an executor via vote_candidate_id column.
func (o *Candidate) VoteCandidateUsers(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`users`.`vote_candidate_id`=?", o.ID),
	)
...
// LoadVoteCandidateUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (candidateL) LoadVoteCandidateUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCandidate interface{}, mods queries.Applicator) error {
	var slice []*Candidate
	...
	query := NewQuery(
		qm.From(`users`),
		qm.WhereIn(`users.vote_candidate_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.VoteCandidateUsers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &userR{}
			}
			foreign.R.VoteCandidate = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.VoteCandidateID) {
				local.R.VoteCandidateUsers = append(local.R.VoteCandidateUsers, foreign)
				if foreign.R == nil {
					foreign.R = &userR{}
				}
				foreign.R.VoteCandidate = local
				break
			}
		}
	}
...
// AddVoteCandidateUsers adds the given related objects to the existing relationships
// of the candidate, optionally inserting them as new records.
// Appends related to o.R.VoteCandidateUsers.
// Sets related.R.VoteCandidate appropriately.
func (o *Candidate) AddVoteCandidateUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.VoteCandidateID, o.ID)
	...
	if o.R == nil {
		o.R = &candidateR{
			VoteCandidateUsers: related,
		}
	} else {
		o.R.VoteCandidateUsers = append(o.R.VoteCandidateUsers, related...)
	}
	...
	for _, rel := range related {
		if rel.R == nil {
			...
			rel.R.VoteCandidate = o
		}
	}
	return nil
}

// SetVoteCandidateUsers removes all previously related items of the
// candidate replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.VoteCandidate's VoteCandidateUsers accordingly.
// Replaces o.R.VoteCandidateUsers with related.
// Sets related.R.VoteCandidate's VoteCandidateUsers accordingly.
func (o *Candidate) SetVoteCandidateUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "update `users` set `vote_candidate_id` = null where `vote_candidate_id` = ?"
	...
	if o.R != nil {
		for _, rel := range o.R.VoteCandidateUsers {
			queries.SetScanner(&rel.VoteCandidateID, nil)
			if rel.R == nil {
				continue
			}

			rel.R.VoteCandidate = nil
		}
		o.R.VoteCandidateUsers = nil
	}

	return o.AddVoteCandidateUsers(ctx, exec, insert, related...)
}

// RemoveVoteCandidateUsers relationships from objects passed in.
// Removes related items from R.VoteCandidateUsers (uses pointer comparison, removal does not keep order)
// Sets related.R.VoteCandidate.
func (o *Candidate) RemoveVoteCandidateUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("vote_candidate_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.VoteCandidateUsers {
			if rel != ri {
				continue
			}

			ln := len(o.R.VoteCandidateUsers)
			if ln > 1 && i < ln-1 {
				o.R.VoteCandidateUsers[i] = o.R.VoteCandidateUsers[ln-1]
			}
			o.R.VoteCandidateUsers = o.R.VoteCandidateUsers[:ln-1]
			break
		}
	}
